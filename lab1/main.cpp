#include <iostream>
#include "shop.h"
#include "seller.h"
#include "product.h"
#include <memory>

int main() {
    auto vapeShop = make_unique<Shop>("Scam_Judas");

    auto seller1 = make_unique<Seller>("Данила", 800);
    auto seller2 = make_unique<Seller>("Яна", 1000, true);
    vapeShop->addSeller(std::move(seller1));
    vapeShop->addSeller(std::move(seller2));    

    auto product1 = make_unique<Product>("Xros 3 mini", 85, 54, 3);
    auto product2 = make_unique<Product>("Hotspot", 15, 8, 60);
    vapeShop->addProduct(std::move(product1));
    vapeShop->addProduct(std::move(product2));

    std::cout << "Информация для обычного продавца (Данила):" << std::endl;
    vapeShop->displayShop(seller1->getIsAdmin());

    std::cout << "\nИнформация для администратора (Яна):" << std::endl;
    vapeShop->displayShop(seller2->getIsAdmin());

    vapeShop->removeSeller("Данила");

    vapeShop->removeProduct("Hotspot");

    vapeShop->displayShop(seller2->getIsAdmin());

    return 0;
}